...
- **Honest-failure invariant** — an error path may never render a numeric cohort. Validator rejection → one resynthesis retry with the violation appended, then honest error (explanation + rejected SQL + reason). Execution error → honest error immediately, no retry. Test-enforced; the direct lesson of #76's swallowed exception rendering cohort = 0.
