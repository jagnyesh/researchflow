...
**Post-6.7 follow-ups (2026-07-13):** #116 CLOSED (PR #120) — retired the `claude-3-7-sonnet-20250219` landmine, which had two homes: the dead `langchain_agents.py` LangChain-migration prototype (deleted whole, ~1,200 LOC) and — the live one — `app/utils/multi_llm_client.py` (6 sites), which the production `calendar_agent`/`delivery_agent` invoke via `langgraph_workflow`. It was 404ing live (secondary=ollama down → Claude fallback → retired model → 404; the likely cause of the "calendar/delivery 5-7ms no-op spans"). Fixed to a single `CLAUDE_MODEL="claude-sonnet-4-6"` constant + wire-level regression test. The `/diagnose` first-pass called it "latent"; the fresh-context review grepping the whole tree caught the live instance — lesson recorded: a model-id landmine check must grep the whole codebase, not just the file the issue names. Also restored the local HAPI corpus from a `drive_fhir_traffic.py` synthetic-write drift (14 patients, 366→380; soft-deleted the post-anchor batch + re-materialized; follow-up to make that writer self-clean filed in BACKLOG).
**Workflow (2026-07-19):** #128 pipeline audit closed — merge-actor rule + SHA-bound review attestation + merge-by-class shipped in PR #130 (`3f28b90`; dogfooded on itself, human-merged per the self-amendment carve-out); cycle-time dials at `scripts/dials.sh`; audit record `docs/audits/2026-07-19-pipeline-audit.md`; lean-ctx §5.5 retro-vet **FAIL**, resolved same day by **PIN** — self-built from source tag v3.9.12, update checks disabled, `lean-ctx update` declared no-go (#134; ledger in CLAUDE.md §5.5). **Ramp position: Stage 2 entered, exit clock unstarted** — Stage-2 exit accrues only from human-attended sessions (first `claude` in the window typed by the human); calendar block pending (audit gap 8).
...
### LLM SQL synthesis — exploratory portal (Sprint 6.7, grilled 2026-07-11; ADR 0028)

- **SQL Synthesis (exploratory)** — single LLM call NL → `{sql, explanation}` replacing BOTH `QueryInterpreter`'s `QueryIntent` and `JoinQueryBuilder`'s f-string SQL on the exploratory path. Decided over a two-call (interpret → synthesize) design because the intent→SQL seam is exactly where criteria historically get dropped (#76-class bugs). Cost stays at 1 LLM call/query; the schema-context prompt block finally clears Anthropic's caching threshold (folds in the Sprint 8.6 candidate).
- **SQL Validator** — deterministic default-deny gate every synthesized query must pass before execution (8 rules; ADR 0028 decision 4). PHI boundary rule: aggregate-only — every top-level SELECT item must be an aggregate function or a GROUP BY dimension, and dimensions must come from a non-identifying allowlist (gender, age-bucket expressions, clinical_status, `*_display`, `*_code`; never name/phone/email/address/id columns). Preserves the existing "COUNT queries only (no PHI exposure)" exploratory contract; load-bearing while #39 (zero dashboard auth) stays open. Small-cell suppression (<11) deliberately deferred to a filed follow-on. Column knowledge AND the LLM's schema prompt block come from live `information_schema` introspection at startup — one source of truth; kills the #76 drift class.
- **Honest-failure invariant** — an error path may never render a numeric cohort. Validator rejection → one resynthesis retry with the violation appended, then honest error. Execution error → honest error immediately, no retry. Test-enforced; the direct lesson of #76's swallowed exception rendering cohort = 0. **Error-variant shape (#96, consumed by #97's UI): `{status:"error", explanation, rejected_sql, reason, time_period, executed_at}` — no `estimated_cohort` key.** Success returns `{status:"ok", estimated_cohort, generated_sql, ...}`.
- **Anchor disclosure** — validated SQL executes via `db_client` under a `SELECT`-only `rf_readonly` role scoped to schema `sqlonfhir`; the validator's parsed view list feeds `get_batch_anchor_ts_for_views()` and the UI renders "data as of `<ts>`". The exploratory portal is and remains batch-only reads — now disclosed instead of implicit. (Naming caveat: the `EXPLORATORY` FreshnessAnnotation mode never had a production caller on this portal; see ADR 0028 impact ledger.)
- **Sprint 6.7 gate (pre-committed)** — flag `USE_LLM_SQL_SYNTHESIS` flips only when: exec accuracy ≥90% on the ~30-case same-run-oracle eval set AND zero validator escapes on the ~10-case adversarial subset (absolute) AND honest-failure invariant green. Model rule: |Sonnet−Opus| ≤ 5 pts → Sonnet; else higher accuracy. Evidence artifact: `logs/sprint_6_7_gate.jsonl`.
