...
class TestSQLStructure:
    ...
    def test_count_query_structure(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("diabetes diagnosis")],
            time_period={"start": "2024-01-01", "end": "2024-12-31"},
        )

        sql = self.sql_generator.generate_phenotype_sql(requirements, count_only=True)
    ...
    def test_full_query_structure(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("heart failure")],
            data_elements=["demographics", "lab_results"],
            time_period={"start": "2024-01-01", "end": "2024-12-31"},
        )

        sql = self.sql_generator.generate_phenotype_sql(requirements, count_only=False)
        ...
        assert any(
            field in sql_lower for field in ["patient_id", "id", "birthdate", "gender"]
    ...
    def test_balanced_parentheses(self):
        ...
        test_cases = [
            RB.build_requirements(
                inclusion=[RB.build_condition("diabetes"), RB.build_condition("hypertension")],
                exclusion=[RB.build_condition("pregnancy")],
            ),
            RB.build_requirements(
                inclusion=[
            ...
            RB.build_requirements(
                inclusion=[RB.build_condition("cancer diagnosis")],
...
class TestInclusionCriteria:
    ...
    def test_single_condition_criterion(self):
        ...
        requirements = RB.build_requirements(inclusion=[RB.build_condition("diabetes mellitus")])
    ...
    def test_multiple_conditions(self):
        ...
        if criteria_found < 3:
            print(f"   ⚠️  WARNING: Not all criteria detected " f"(found {criteria_found}/3)")
    ...
    def test_age_criterion(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_demographic("age > 65", term="age", details="> 65")]
        )
...
class TestExclusionCriteria:
    ...
    def test_exclusion_criteria(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("diabetes")],
            exclusion=[
        ...
        has_exclusion_logic = any(
            term in sql_lower for term in ["not exists", "not in", "where not"]
        )
        ...
        assert "diabetes" in sql_lower or "e11" in sql_lower, "Inclusion criteria missing"
...
class TestTimePeriodFilters:
    ...
    def test_date_range_filter(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("COVID-19 diagnosis")],
        ...
        has_date_filter = any(
            term in sql_lower for term in ["recordeddate", "onsetdate", "date", "between"]
        )
    ...
    def test_start_date_only(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("hypertension")], time_period={"start": "2024-01-01"}
        )
...
class TestDataElements:
    ...
    def test_demographics_only(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("diabetes")], data_elements=["demographics"]
        )
        ...
        has_demographics = any(
            field in sql_lower for field in ["birthdate", "gender", "race", "ethnicity"]
        )
    ...
    def test_multiple_data_elements(self):
        ...
        requirements = RB.build_requirements(
            inclusion=[RB.build_condition("heart failure")],
            data_elements=["demographics", "lab_results", "medications", "procedures"],
        )
...
class TestComplexScenarios:
    ...
    def test_complex_oncology_query(self):
        ...
        requirements = RB.build_requirements(
            ...
            exclusion=[RB.build_condition("pregnancy"), RB.build_condition("hospice care")],
            data_elements=["demographics", "diagnoses", "medications", "procedures", "lab_results"],
        ...
        print(
            f"Time Period: {requirements['time_period']['start']} to "
