...
class TestMultiLLMClientInitialization:
    ...
    def test_init_with_anthropic_only(self):
        ...
        with patch.dict(
            os.environ, {"ANTHROPIC_API_KEY": "test-key", "SECONDARY_LLM_PROVIDER": "anthropic"}
        ):
    ...
    def test_init_openai_without_api_key(self):
        ...
        with patch.dict(
            os.environ,
            {"ANTHROPIC_API_KEY": "test-key", "SECONDARY_LLM_PROVIDER": "openai"},
            clear=True,
        ):
    ...
    def test_fallback_can_be_disabled(self):
        ...
        with patch.dict(
            os.environ, {"ANTHROPIC_API_KEY": "test-key", "ENABLE_LLM_FALLBACK": "false"}
        ):
...
class TestModelIdentifierSelection:
    ...
    def test_critical_tasks_use_claude(self):
        ...
        with patch.dict(
            os.environ, {"ANTHROPIC_API_KEY": "test-key", "SECONDARY_LLM_PROVIDER": "openai"}
        ):
            client = MultiLLMClient()
...
class TestCompleteMethod:
    ...
    async def test_critical_task_uses_claude_client(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            result = await client.complete(
                prompt="Test prompt", task_type="requirements"  # Critical task
            )
    ...
    async def test_non_critical_task_uses_secondary_provider(self, mock_aisuite):
        ...
        with patch.dict(
            ...
            result = await client.complete(
                prompt="Test prompt", task_type="calendar"  # Non-critical task
            )
    ...
    async def test_fallback_to_claude_on_error(self, mock_aisuite):
        ...
        with patch.dict(
            ...
            result = await client.complete(prompt="Test prompt", task_type="calendar")
    ...
    async def test_no_fallback_raises_error(self, mock_aisuite):
        ...
        with patch.dict(
            ...
            with pytest.raises(Exception, match="Provider error"):
                await client.complete(prompt="Test prompt", task_type="calendar")
...
class TestExtractStructuredJSON:
    ...
    async def test_critical_task_uses_claude_client(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            result = await client.extract_structured_json(
                prompt="Extract requirements", schema_description="Schema", task_type="requirements"
            )
    ...
    async def test_non_critical_task_json_parsing(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            result = await client.extract_structured_json(
                prompt="Test", schema_description="Schema", task_type="calendar"
            )
...
class TestWrapperMethods:
    ...
    async def test_extract_requirements_uses_claude(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            ...
            result = await client.extract_requirements(
                conversation_history=[], current_requirements={}
            )
    ...
    async def test_extract_medical_concepts_uses_claude(self):
        ...
        with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key"}):
            client = MultiLLMClient()
            client.claude_client.extract_medical_concepts = AsyncMock(return_value={"concepts": []})
