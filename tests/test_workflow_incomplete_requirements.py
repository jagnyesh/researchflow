...
class TestIncompleteRequirementsWorkflow:
    ...
    def test_incomplete_requirements_continues_gathering(self):
        ...
        assert next_step['next_agent'] == 'requirements_agent', "Should continue with requirements agent"
        assert next_step['next_task'] == 'gather_requirements', "Should continue gathering requirements"
        assert next_step['next_state'] == WorkflowState.REQUIREMENTS_GATHERING, "Should stay in gathering state"
    ...
    def test_complete_requirements_goes_to_review(self):
        ...
        assert next_step['next_state'] == WorkflowState.REQUIREMENTS_REVIEW, "Should move to review state"
    ...
    def test_workflow_supports_multiple_conditions(self):
        ...
        assert len(rule) >= 3, "Should have at least 3 conditions (complete+approval, complete+no approval, incomplete)"

        # Assert first condition is for complete requirements WITH approval needed (conversational mode)
        assert rule[0]['condition']({'requirements_complete': True, 'requires_approval': True}), \
            "First condition should match complete=True and requires_approval=True"
        assert rule[0]['next_state'] == WorkflowState.REQUIREMENTS_REVIEW, "Complete with approval should go to review"

        # Assert second condition is for complete requirements WITHOUT approval (form-based mode)
        assert rule[1]['condition']({'requirements_complete': True, 'requires_approval': False}), \
            "Second condition should match complete=True and requires_approval=False"
        assert rule[1]['next_state'] == WorkflowState.FEASIBILITY_VALIDATION, "Complete without approval should go to phenotype"

        # Assert third condition is for incomplete requirements
        assert rule[2]['condition']({'requirements_complete': False}), "Third condition should match complete=False"
        assert rule[2]['next_state'] == WorkflowState.REQUIREMENTS_GATHERING, "Incomplete should stay in gathering"
    ...
    def test_phenotype_always_goes_to_review(self):
        ...
        assert rule['next_state'] == WorkflowState.PHENOTYPE_REVIEW, "Should always go to phenotype review"
