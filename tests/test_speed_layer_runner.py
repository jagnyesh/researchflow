...
async def test_execute_empty_cache(speed_runner, patient_view_def):
    ...
    # Query empty cache
    result = await speed_runner.execute(view_definition=patient_view_def, max_resources=100)
...
async def test_gender_filter(redis_client, speed_runner, patient_view_def, sample_patients):
    ...
    result = await speed_runner.execute(
        view_definition=patient_view_def, search_params={"gender": "male"}, max_resources=100
    )
    ...
    result_female = await speed_runner.execute(
        view_definition=patient_view_def, search_params={"gender": "female"}, max_resources=100
    )

    assert (
        result_female["total_count"] == 1
    ), f"❌ Expected 1 female patient, got {result_female['total_count']}"
...
@pytest.mark.asyncio
async def test_condition_resources(
    redis_client, speed_runner, condition_view_def, sample_conditions
):
    ...
    for condition in sample_conditions:
        await redis_client.set_fhir_resource("Condition", condition["id"], condition, ttl_hours=24)

    print(f"✓ Cached {len(sample_conditions)} conditions in Redis")

    # Query speed layer
    result = await speed_runner.execute(view_definition=condition_view_def, max_resources=100)
    ...
    assert (
        actual_patient_ids == expected_patient_ids
    ), f"❌ Patient IDs don't match: {actual_patient_ids}"
...
async def test_code_filter(redis_client, speed_runner, condition_view_def, sample_conditions):
    ...
    result = await speed_runner.execute(
        view_definition=condition_view_def, search_params={"code": "73211009"}, max_resources=100
    )

    # Should find 2 diabetes conditions
    assert (
        len(result["resources"]) == 2
    ), f"❌ Expected 2 diabetes conditions, got {len(result['resources'])}"
    ...
    result_htn = await speed_runner.execute(
        view_definition=condition_view_def, search_params={"code": "38341003"}, max_resources=100
    )

    assert (
        len(result_htn["resources"]) == 1
    ), f"❌ Expected 1 hypertension condition, got {len(result_htn['resources'])}"
...
async def test_time_filter(redis_client, speed_runner, patient_view_def, sample_patients):
    ...
    result = await speed_runner.execute(
        view_definition=patient_view_def, since=cutoff_time, max_resources=100
    )
...
async def test_max_resources_limit(redis_client, speed_runner, patient_view_def):
    ...
    result = await speed_runner.execute(view_definition=patient_view_def, max_resources=5)
    ...
    result_all = await speed_runner.execute(view_definition=patient_view_def, max_resources=100)

    assert (
        len(result_all["resources"]) == 10
    ), f"❌ Expected 10 resources, got {len(result_all['resources'])}"
...
async def test_code_text_matching(redis_client, speed_runner, condition_view_def):
    ...
    result = await speed_runner.execute(
        view_definition=condition_view_def, search_params={"code": "diabetes"}, max_resources=100
    )
    ...
    result_code = await speed_runner.execute(
        view_definition=condition_view_def, search_params={"code": "E11.9"}, max_resources=100
    )
...
async def test_multiple_resource_types(
    ...
    # Query patients
    patient_result = await speed_runner.execute(view_definition=patient_view_def, max_resources=100)

    # Query conditions
    condition_result = await speed_runner.execute(
        view_definition=condition_view_def, max_resources=100
    )
